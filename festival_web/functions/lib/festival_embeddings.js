...
exports.onFestivalProfilePhotoUpdated = exports.seedFestivalEmbeddings = exports.seedFestivalEmbeddingsHttp = void 0;
exports.seedAllFestivalEmbeddings = seedAllFestivalEmbeddings;
const https_1 = require("firebase-functions/v2/https");
const firestore_1 = require("firebase-functions/v2/firestore");
const logger = __importStar(require("firebase-functions/logger"));
const firestore_2 = require("firebase-admin/firestore");
const db = (0, firestore_2.getFirestore)();
const PROJECT_BUCKET = process.env.FIREBASE_STORAGE_BUCKET ||
...
async function writeAiEmbedding(code) {
...
async function writeProfileEmbedding(ticketId, photoUrl) {
...
async function seedAllFestivalEmbeddings() {
    const codes = [];
    for (let i = 1; i <= 20; i++) {
        codes.push(`f${i}`, `m${i}`);
    }
...
exports.seedFestivalEmbeddingsHttp = (0, https_1.onRequest)({
    ...
    try {
        ...
        const { generateRecommendationsForTicket } = await Promise.resolve().then(() => __importStar(require("./festival_recommendations.js")));
        ...
        for (const doc of ticketsSnap.docs) {
            try {
                const rec = await generateRecommendationsForTicket(doc.id, "embedding_seed");
                if (rec.success)
                    recSuccess += 1;
            }
            catch (error) {
...
exports.seedFestivalEmbeddings = (0, https_1.onCall)({
    region: "asia-northeast3",
    memory: "2GiB",
    timeoutSeconds: 540,
    maxInstances: 1,
...
exports.onFestivalProfilePhotoUpdated = (0, firestore_1.onDocumentWritten)({
    ...
    const beforeUrl = event.data?.before?.exists
        ...
        : "";
    if (beforeUrl === photoUrl && event.data?.before?.exists)
        return;
    try {
...
//# sourceMappingURL=festival_embeddings.js.map