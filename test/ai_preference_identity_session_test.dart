...
void main() {
  ...
  for (final shotIndex in [0, 1, 2]) {
    test('Heart on shot ${shotIndex + 1} commits one identity LIKE and advances',
        () async {
  ...
  test('X commits one identity NOPE and advances without deciding remaining shots',
      () {
    final writes = <AiPreferenceDecisionCommit>[];
    final session = AiPreferenceIdentitySessionController(
  ...
  test('duplicate buttons, swipe races, and duplicate animation callbacks commit once',
      () {
    final writes = <AiPreferenceDecisionCommit>[];
    final session = AiPreferenceIdentitySessionController(
      identities: [_identity('male_007'), _identity('female_123')],
  ...
  test('decision is terminal and cannot be changed by browsing remaining shots', () {
    final session = AiPreferenceIdentitySessionController(
      identities: [_identity('male_007'), _identity('female_123')],
    );
  ...
  test('one identity order is stable while the next identity gets its own order', () {
    ...
    final firstOrder = session.currentShots.map((shot) => shot.shotType).toList();
    session.browseNextShot();
    final sameIdentityOrder =
        session.currentShots.map((shot) => shot.shotType).toList();
    expect(sameIdentityOrder, firstOrder);

    session.submitDecision(eventType: 'like');
    session.advanceAfterDecision('male_007');
    final nextOrder = session.currentShots.map((shot) => shot.shotType).toList();
