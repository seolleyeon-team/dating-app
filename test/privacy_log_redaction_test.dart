...
void main() {
  test('global runtime print/debugPrint calls avoid raw privacy values', () {
    final findings = <String>[];

    for (final file in Directory('lib').listSync(recursive: true)) {
      if (file is! File || !file.path.endsWith('.dart')) continue;
      final text = file.readAsStringSync();
      for (final finding in _unsafeLogFindings(text)) {
...
''';
  ...
  test('local redactor names are not automatically trusted', () {
    const source = r'''
void unsafe(Object error) {
  debugPrint('error=${_redactError(error)}');
}
''';

    expect(_unsafeLogFindings(source), hasLength(1));
  });
  test('privacy log utility fingerprints do not contain raw input', () {
    const rawId = 'kakao_user_1234567890';
    const rawPath = 'ai_profiles/female/251.png?token=secret';

    final id = PrivacyLogUtils.idFingerprint(rawId);
    final path = PrivacyLogUtils.pathFingerprint(rawPath);

    expect(id, matches(RegExp(r'^idHash=[a-f0-9]{12}$')));
    expect(path, matches(RegExp(r'^pathHash=[a-f0-9]{12}$')));
    expect(id, isNot(contains(rawId)));
    expect(path, isNot(contains(rawPath)));
    expect(PrivacyLogUtils.idFingerprint(rawId), id);
    expect(PrivacyLogUtils.pathFingerprint(rawPath), path);
  });

  test('privacy log utility summarizes errors without raw messages', () {
    final summary = PrivacyLogUtils.errorSummary(
      FirebaseException(
        plugin: 'firebase_storage',
        code: 'storage/object-not-found',
        message:
            'https://seolleyeon-festival-private-source-photos.storage.googleapis.com/users/kakao_user_123/source.jpg?token=secret',
      ),
    );

    expect(summary, contains('errorType=FirebaseException'));
    expect(summary, contains('code=storage/object-not-found'));
    expect(summary, isNot(contains('kakao_user_123')));
    expect(summary, isNot(contains('token=secret')));
    expect(summary, isNot(contains('source.jpg')));
  });
}

final _safePrivacyLogWrapper = RegExp(
  r'\b(?:PrivacyLogUtils\.\w+|FirebaseDiagnostics\.safeErrorForLog)\s*\(',
  caseSensitive: false,
);
...
final _mapSensitiveKey = RegExp(
  r'''\[\s*['"](?:uid|userId|kakaoUserId|firebaseUid|email|nickname|phone|authorId|writerId|token|url|uri|storagePath|storageUrl|path|gcsUri|source|sourceUrl|sourcePhotoUrl)['"]\s*\]''',
  caseSensitive: false,
);
