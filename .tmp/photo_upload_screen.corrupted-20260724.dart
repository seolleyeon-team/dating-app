...
import '../widgets/avatar_source_consent_controls.dart';
...
class PhotoUploadScreen extends StatefulWidget {
  ...
  /// ?ÑÎ∞î?Ä ?ùÏÑ±/?πÏù∏ ?êÎ¶Ñ???¥Îãπ?òÎäî ?¥Îùº?¥Ïñ∏?? Í∏∞Î≥∏Í∞íÏ? Î∞±Ïóî??ÏΩúÎü¨Î∏îÏùÑ
  /// ?∏Ï∂ú?òÎäî [BackendAvatarGenerationClient]?¥Î©∞, ?ÑÏ†Ø ?åÏä§???îÏûê??QA?êÏÑúÎß?  /// [MockAvatarGenerationClient]Î•?Ï£ºÏûÖ???¨Ïö©?©Îãà??
...
class _PhotoUploadScreenState extends State<PhotoUploadScreen> {
  ...
  AvatarSourceConsent _avatarSourceConsent = const AvatarSourceConsent();
  ...
  UserProfileReader get _users => _userProfileReader ??=
  ...
  Future<void> _loadExistingPhotos() async {
    ...
    try {
      kakaoUserId = await _storage.getKakaoUserId();
      if (kakaoUserId == null || kakaoUserId.isEmpty) return;
      data = await _users.getUserProfile(kakaoUserId);
    } catch (error) {
      _logAvatarFlow('avatar_initial_profile_failed', error: error);
      return;
    }
  ...
  Future<void> _refreshCurrentAvatarGenerationStatus({
    ...
    try {
      ...
      if (!mounted) return;
      if (status.approved || status.status == AvatarJobStatus.approved) {
  ...
  Future<void> _handleNext() async {
    ...
    if (_photoCount < _minRequiredPhotos &&
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('?¨ÏßÑ??ÏµúÏÜå 1???¥ÏÉÅ ?±Î°ù?¥Ï£º?∏Ïöî.'),
          behavior: SnackBarBehavior.floating,
        ),
      );
      return;
    ...
    if (_isUploading.any((e) => e)) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('?¨ÏßÑ ?ÖÎ°ú?úÍ? ?ùÎÇú ???§Ïùå?ºÎ°ú ?òÏñ¥Í∞ÄÏ£ºÏÑ∏??'),
          behavior: SnackBarBehavior.floating,
        ),
      );
      return;
  ...
  Future<String?> _uploadLocalPrimarySource(int index) async {
    ...
    if (kakaoUserId == null || kakaoUserId.isEmpty) {
      throw Exception('?¨Ïö©???ïÎ≥¥Î•?Ï∞æÏùÑ ???ÜÏäµ?àÎã§. ?§Ïãú Î°úÍ∑∏?∏Ìï¥Ï£ºÏÑ∏??');
    }
    ...
    try {
      ...
      final result = await _avatarClient.uploadSourcePhoto(
        ...
        avatarSourceConsent: _avatarSourceConsent,
      );
    ...
    try {
      ...
      if (result.status == AvatarJobStatus.failed ||
        ...
        _failAvatarGeneration(
          ...
          jobId: jobId,
          retryAllowed: _authoritativeRetryAllowed(result),
        );
        return;
      }
  ...
  Future<void> _approveAvatarCandidate({
    ...
    try {
      ...
      setState(() {
        _avatarFlowState = AvatarOnboardingFlowState.previewReady;
        _avatarApprovalError = '?ÑÎ∞î?Ä ?Ä?•Ïóê ?§Ìå®?àÏñ¥?? ?§Ïãú ??Î≤??†ÌÉù?¥Ï£º?∏Ïöî.';
      });
      refreshDialog(() {});
    }
  }
  ...
  bool _authoritativeRetryAllowed(AvatarCandidatesResult result) {
    try {
      return (result as dynamic).retryAllowed == true;
    ...
    return showDialog<bool>(
      ...
      builder: (ctx) {
        return AlertDialog(
          ...
          content: const Text(
            'ÏßÄÍ∏??òÍ?Î©??ÑÎ∞î?Ä ?ùÏÑ±??Ï§ëÎã®?ºÏöî.\n?òÍ??úÍ≤†?¥Ïöî?',
            style: TextStyle(fontFamily: 'Pretendard'),
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(ctx).pop(false),
              child: const Text('?®Íæ©??Êπ≤Í≥ï?éÁî±?Î¶?),
            ),
            TextButton(
              onPressed: () => Navigator.of(ctx).pop(true),
              child: const Text('?òÍ?Í∏?),
            ),
          ],
        );
      },
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 24),
                          AvatarSourceConsentControls(
                            value: _avatarSourceConsent,
                            locked: _isSourceMutationBlocked,
                            onChanged: (value) {
                              if (_avatarLocked) {
                                _showLockedAvatarMessage();
                                return;
                          ...
                          Row(
                            ...
                            children: const [
                              ...
                              Expanded(
                                child: Text(
                                  'Î≥∏Ïù∏???òÏò§ÏßÄ ?äÍ±∞??Î∂àÏæåÍ∞êÏùÑ Ï£ºÎäî ?¨ÏßÑ?Ä ?µÎ≥¥ ?ÜÏù¥ ??†ú?????àÏäµ?àÎã§.',
                                  style: TextStyle(
                                    fontFamily: 'Pretendard',
                                    fontSize: 12,
                                    color: _AppColors.textSub,
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: const [
        ...
        SizedBox(height: 8),
        Text(
          '?ºÍµ¥?????òÏò® ?¨ÏßÑ?ºÏàòÎ°?Îß§Ïπ≠ ?ïÎ•†???¨ÎùºÍ∞Ä??,
          style: TextStyle(
            fontFamily: 'Pretendard',
            fontSize: 14,
            color: _AppColors.textSub,
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                Text(
                  'Ï±ÑÌåÖ ?ÅÎ??êÍ≤å ?§Ï†ú ?ÑÎ°ú???¨ÏßÑ Í≥µÍ∞ú ?ôÏùò',
                  style: TextStyle(
                    fontFamily: 'Pretendard',
                    fontSize: 13,
                    fontWeight: FontWeight.w700,
                ...
                SizedBox(height: 6),
                Text(
                  'Ï∂îÏ≤ú ?îÎ©¥?êÎäî ?†ÌÉù???ÑÎ∞î?ÄÍ∞Ä ?úÏãú?ºÏöî. Ï±ÑÌåÖÎ∞©Ïù¥ ÎßåÎì§?¥ÏßÑ ?ÅÎ??êÍ≤å???§Ï†ú ?ÑÎ°ú???¨ÏßÑ???úÏãú?????àÍ≥†, ?êÎ≥∏ ?¨ÏßÑ?Ä Ï∂îÏ≤ú Ïπ¥Îìú??Í≥µÍ∞ú ?ÑÎ°ú?ÑÏóê???úÏãú?òÏ? ?äÏïÑ??',
                  style: TextStyle(
                    fontFamily: 'Pretendard',
                    fontSize: 12,
                    color: _AppColors.textSub,
