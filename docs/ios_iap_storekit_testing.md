...
3. 이 파일에 있는 세 상품은 모두 Consumable입니다.
...
`IAP_VERIFICATION_MODE`의 기본값은 `production`이며, 이 모드의 `ProductionApplePurchaseVerifier`는 검증 구현 전에는 지급을 거부합니다. 운영/staging 배포 환경에 `storekit_local`을 넣지 마세요. 실제 App Store Connect 전환 때는 이 verifier에 App Store Server API 또는 App Store Server Library JWS 검증을 구현하면 Flutter/UI/Firestore idempotency 구조는 그대로 재사용됩니다.

## 확인 시나리오

- 10/30/100 하트 상품을 각각 구매하면 `users/{kakaoUserId}.heartBalance`가 정확히 증가합니다.
