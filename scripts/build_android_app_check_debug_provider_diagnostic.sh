#!/bin/zsh
# Creates a temporary internal-test AAB that uses Firebase App Check's Android
# Debug Provider. Do not upload this artifact to production or public tracks.
...
cleanup() {
  ...
  unset FIREBASE_APP_CHECK_DEBUG_TOKEN
}
trap cleanup EXIT
...
read -r -s "FIREBASE_APP_CHECK_DEBUG_TOKEN?Firebase에 등록한 App Check Debug Token을 입력하세요: "
print

if [[ -z "$FIREBASE_APP_CHECK_DEBUG_TOKEN" || "$FIREBASE_APP_CHECK_DEBUG_TOKEN" == *[[:space:]]* ]]; then
  print -u2 "공백 없는 Firebase App Check Debug Token을 입력해야 합니다."
  exit 1
fi
...
archive_name="Seolleyeon-${version//+/_}-app-check-debug-provider-diagnostic.aab"
...
flutter build appbundle --release \
  ...
  --dart-define="APP_CHECK_ANDROID_DEBUG_TOKEN=$FIREBASE_APP_CHECK_DEBUG_TOKEN" \
  --dart-define=APP_CHECK_DIAGNOSTIC_MODE=true \
  --dart-define="APP_CHECK_DIAGNOSTIC_BUILD_LABEL=$version-debug-provider"

[[ -f "$artifact_path" ]] || { print -u2 "AAB 출력 파일을 찾지 못했습니다."; exit 1; }
mkdir -p "$archive_dir"
cp "$artifact_path" "$archive_path"
sha256=$(shasum -a 256 "$archive_path" | awk '{print $1}')
print "Debug Provider 진단 AAB 생성 완료"
print "버전: $version"
