...
class TeamMeetingRequestService {
  ...
  Future<String> createMeetingRequest({
    ...
    try {
      ...
      if (requestId.isEmpty) {
        throw StateError('미팅 요청을 만들지 못했어요.');
      }
      return requestId;
  ...
  Future<String> acceptRequest(String requestId) async {
    try {
      ...
      if (matchId.isEmpty) {
        throw StateError('미팅 연결 정보를 확인하지 못했어요.');
      }
      return matchId;
    } on FirebaseFunctionsException catch (error) {
      throw StateError(_functionsErrorMessage(error));
  ...
  String _functionsErrorMessage(FirebaseFunctionsException error) {
    switch (error.code) {
      case 'invalid-argument':
        return '요청 정보가 올바르지 않아요.';
      case 'already-exists':
        return '이미 처리 중인 미팅 요청이 있어요.';
      case 'not-found':
        return '미팅 요청을 찾을 수 없어요.';
      case 'permission-denied':
      case 'unauthenticated':
        return '이 요청을 처리할 권한이 없어요.';
      case 'failed-precondition':
      case 'aborted':
        return '요청 상태가 변경됐어요. 다시 확인해주세요.';
      case 'resource-exhausted':
      case 'unavailable':
        return '잠시 후 다시 시도해주세요.';
      default:
        return '미팅 요청을 처리하지 못했어요.';
    }
  }
