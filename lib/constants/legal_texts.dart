...
class LegalTexts {
  ...
  static const version = '2026-05-16';
  ...
  static const serviceTerms = LegalTextDocument(
    ...
    sections: [
      ...
      LegalTextSection(
        ...
        content:
            '이용자는 청소년보호법상 청소년에 해당하지 않는 이용자임을 확인하기 위한 본인인증 및 성인 여부 확인을 완료한 뒤 카카오 로그인 등 회사가 제공하는 인증 방식을 통해 회원가입할 수 있습니다. 본인인증 정보와 카카오 로그인 정보는 서로 다른 목적의 인증 정보이며, 이용자는 정확한 정보를 제공해야 하고 타인의 정보를 도용하거나 허위 정보를 입력해서는 안 됩니다.',
      ),
      LegalTextSection(
        title: '제4조 이용자의 의무',
        content:
  ...
  static const privacyPolicy = LegalTextDocument(
    ...
    sections: [
      ...
      LegalTextSection(
        title: '4. 본인인증 정보의 공개 여부',
        content:
            '본인인증으로 확인한 이름, 휴대전화번호, 생년월일, CI/DI 또는 이에 준하는 식별값은 성인 여부 확인, 본인 확인, 중복 가입 방지, 신고 및 분쟁 대응, 부정 이용 방지 목적으로만 사용되며 다른 사용자에게 공개되지 않습니다.',
      ),
      LegalTextSection(
      ...
      LegalTextSection(
        title: '6. 개인정보 처리 위탁',
        content:
            '서비스 운영을 위해 Firebase, Google Cloud, Kakao, PortOne 및 본인인증 제공사 등 외부 플랫폼을 이용할 수 있으며, 이 과정에서 서비스 제공에 필요한 범위 내에서 개인정보가 처리될 수 있습니다.',
      ),
      LegalTextSection(
        title: '7. 이용자의 권리',
        content:
  ...
  static const kakaoNamePhoneConsent = LegalTextDocument(
    id: 'kakaoNamePhone',
    title: '본인인증 정보 수집·이용 동의',
    summary: '본인인증 및 성인 여부 확인을 위한 이름, 전화번호, 생년월일, 식별값 이용 목적 안내',
    sections: [
      LegalTextSection(
        title: '본인인증 정보 수집·이용 동의',
        content:
            '설레연은 대학생 인증 기반 매칭 커뮤니티 서비스의 안전한 운영과 청소년 이용 제한을 위해 카카오 로그인 전에 본인확인기관 기반 본인인증을 진행합니다. 이름, 휴대전화번호, 생년월일 및 본인인증 결과는 본인확인기관 기반 본인인증 과정에서 확인·수집될 수 있으며, 실사용자 확인, 성인 여부 확인, 중복 가입 방지, 신고 및 분쟁 대응 등 안전한 서비스 운영을 위해 사용됩니다. 본인인증 정보와 카카오 로그인 정보는 목적이 다르며, 인증된 정보는 다른 사용자에게 공개되지 않습니다.',
      ),
      LegalTextSection(
        title: '1. 수집 항목',
        content: '- 이름\n- 휴대전화번호\n- 생년월일\n- 성인인증 결과\n- CI/DI 또는 이에 준하는 식별값',
      ...
      LegalTextSection(
        ...
        content:
            '- 사용자는 본인인증 정보 수집·이용에 대한 동의를 거부할 수 있습니다.\n- 다만 해당 정보는 안전한 서비스 운영, 청소년 이용 제한 및 사용자 보호를 위한 필수 항목이므로, 동의하지 않을 경우 설레연 회원가입 및 서비스 이용이 제한될 수 있습니다.',
      ),
      LegalTextSection(
        title: '5. 동의 여부 저장',
  ...
  static const ageOver20Consent = LegalTextDocument(
    id: 'ageOver20',
    title: '성인 이용자 확인 동의',
    summary: '청소년보호법상 청소년에 해당하지 않는 이용자임을 확인하는 필수 동의',
    sections: [
      LegalTextSection(
        title: '성인 이용자 확인',
        content:
            '설레연은 대학생 인증 기반 매칭 커뮤니티 서비스로, 청소년보호법상 청소년에 해당하지 않는 이용자에 한해 가입 및 서비스 이용을 허용합니다.',
      ),
      LegalTextSection(
        title: '이용 제한 기준',
        content:
            '본인인증 결과 청소년보호법상 청소년에 해당하거나 연 나이 20세 이상 이용 대상에 해당하지 않는 경우 회원가입 및 서비스 이용이 제한될 수 있습니다.',
      ),
    ],
  );
