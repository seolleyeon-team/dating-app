...
def avatar_job_claims_enabled() -> bool:
    if _env_bool("AVATAR_GPU_WORKER_ENABLED", True) is False:
        return False
    if _env_bool("AVATAR_DISABLE_NEW_GENERATION", False) is True:
        return False
    if _env_bool("AVATAR_COST_KILL_SWITCH_ENABLED", False) is True:
        return False
    return True
...
def _production_environment() -> bool:
    ...
    return environment in {"production", "prod"} or node_env == "production"
...
def _env_bool_any(names: Sequence[str], fallback: bool) -> bool:
    for name in names:
        ...
        if raw is not None and raw.strip():
            return _env_bool(name, fallback)
    return fallback
