...
from .blur_assessment import BlurAssessment, BlurAssessmentConfig, BlurAssessor
...
class SmallFaceSourcePipeline:
    def __init__(
        ...
        *,
        ...
        self._blur_assessment_config = BlurAssessmentConfig(
            min_native_short_side_px=self._config.min_short_side_trait_px,
        )
        self._blur_assessor = BlurAssessor(self._blur_assessment_config)
    ...
    def _observe_blur_shadow(
        self,
        image: Image.Image,
        primary: InternalFaceDetection,
    ) -> tuple[Optional[BlurAssessment], dict[str, object]]:
        metrics: dict[str, object] = {
            "primaryFaceSizeBucket": _size_bucket(primary.face_short_side_px),
        }
        if not self._config.blur_shadow_enabled:
            metrics["blurShadowDecision"] = "disabled"
            return None, metrics

        started = time.perf_counter()
        try:
            assessment = self._blur_assessor.assess(image, primary)
        except Exception:
            assessor_config = self._blur_assessment_config
            metrics.update(
                {
                    "blurAssessmentShadowMs": int(
                        (time.perf_counter() - started) * 1000
                    ),
                    "blurMetricVersion": assessor_config.metric_version,
                    "blurPolicyVersion": assessor_config.policy_version,
                    "blurCalibrationStatus": assessor_config.calibration_status,
                    "blurShadowDecision": "unavailable",
                }
            )
            return None, metrics
        ...
        metrics.update(
            {
                ...
                "blurShadowDecision": assessment.decision,
            }
        )
    ...
    def run(self, image_data) -> SmallFacePipelineResult:
        ...
        if normalized is None:
            metrics["sourceRejectedBeforeGpu"] = True
            analysis = InternalFaceAnalysis(
        ...
        full_detections: List[InternalFaceDetection] = []
        if self._config.full_range_enabled:
        ...
        metrics["detectionPassCount"] = int(self._config.full_range_enabled)
        ...
        if self._tiles.should_run_tiles(
            ...
            used_tile = True
            metrics["detectionPassCount"] = int(metrics["detectionPassCount"]) + 1
        ...
        # Candidate v3 observation is opt-in and non-authoritative. The active v1
        # gate below remains unchanged until calibration and review are complete.
        quality_assessment, shadow_metrics = self._observe_blur_shadow(
            image,
            primary,
        )
        metrics.update(shadow_metrics)

        # Active legacy quality gate (stricter than detect).
